...
import { Literal, Op, Value, } from 'decaffeinate-coffeescript2/lib/coffeescript/nodes';
...
function getElements(node, context) {
    if (node instanceof Op && isImplicitPlusOp(node, context)) {
        ...
        return __spreadArray(__spreadArray([], getElements(node.first, context)), getElements(node.second, context));
    }
    return [node];
}
/**
