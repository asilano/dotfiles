...
var ConditionalPatcher = /** @class */ (function (_super) {
    __extends(ConditionalPatcher, _super);
    ...
    ConditionalPatcher.prototype.patchAsExpression = function (_a) {
        var _b = _a === void 0 ? {} : _a, needsParens = _b.needsParens;
        ...
        var _c = this, consequent = _c.consequent, alternate = _c.alternate;
        if (consequent && alternate) {
            if (!elseToken) {
            ...
            var semicolonTokenIndex = this.indexOfSourceTokenBetweenSourceIndicesMatching(this.condition.outerEnd, elseToken.start, function (token) { return token.type === SourceType.SEMICOLON; });
    ...
    ConditionalPatcher.prototype.patchAsIIFE = function () {
        ...
        var conditionIndent = notNull(this.parent).getIndent(1);
        if (this.consequent) {
    ...
    ConditionalPatcher.prototype.patchConditionForStatement = function () {
        // `unless a` → `if a`
        //  ^^^^^^        ^^
        var ifToken = notNull(this.sourceTokenAtIndex(this.getIfSourceTokenIndex()));
    ...
    ConditionalPatcher.prototype.patchConsequentForStatement = function () {
        this.insert(this.condition.outerEnd, ' {');
        if (this.alternate) {
            var elseTokenIndex = notNull(this.getElseSourceTokenIndex());
            var elseToken = notNull(this.sourceTokenAtIndex(elseTokenIndex));
    ...
    ConditionalPatcher.prototype.patchAlternateForStatement = function () {
        ...
        if (this.alternate && elseTokenIndex) {
            var ifToken = this.sourceTokenAtIndex(notNull(elseTokenIndex.next()));
            var isElseIf = ifToken ? ifToken.type === SourceType.IF : false;
            if (isElseIf) {
    ...
    ConditionalPatcher.prototype.getIfSourceTokenIndex = function () {
        ...
        var ifToken = notNull(this.sourceTokenAtIndex(ifTokenIndex));
    ...
    ConditionalPatcher.prototype.getElseSourceTokenIndex = function () {
        var elseTokenIndex = this.indexOfSourceTokenBetweenSourceIndicesMatching(this.consequent !== null ? this.consequent.outerEnd : this.condition.outerEnd, this.alternate !== null ? this.alternate.outerStart : this.outerEnd, function (token) { return token.type === SourceType.ELSE; });
    ...
    ConditionalPatcher.prototype.getThenTokenIndex = function () {
        ...
        return this.indexOfSourceTokenBetweenSourceIndicesMatching(this.condition.outerEnd, searchEnd, function (token) { return token.type === SourceType.THEN; });
    };
    /**
