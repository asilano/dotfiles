...
var FunctionPatcher = /** @class */ (function (_super) {
    __extends(FunctionPatcher, _super);
    ...
    FunctionPatcher.prototype.patchAsExpression = function () {
        ...
        var assignments = [];
        try {
            for (var _c = __values(this.parameters.entries()), _d = _c.next(); !_d.done; _d = _c.next()) {
                var _e = __read(_d.value, 2), i = _e[0], parameter = _e[1];
                if (firstRestParamIndex === -1 || i < firstRestParamIndex) {
                    ...
                    assignments.push.apply(assignments, __spreadArray([], __read(newAssignments)));
                    neededExplicitBindings.push.apply(neededExplicitBindings, __spreadArray([], __read(newBindings)));
                }
                else {
                    parameter.patch();
                }
        ...
        if (firstRestParamIndex !== -1) {
            if (firstRestParamIndex === this.parameters.length - 1 &&
                ...
                for (var i = firstRestParamIndex; i < this.parameters.length; i++) {
                    neededExplicitBindings.push.apply(neededExplicitBindings, __spreadArray([], __read(getAssigneeBindings(this.parameters[i].node))));
                }
            }
        }
        var uniqueExplicitBindings = __spreadArray([], __read(new Set(neededExplicitBindings)));
    ...
    FunctionPatcher.prototype.patchParameterAndGetAssignments = function (parameter) {
        ...
        this.addDefaultParamAssignmentAtScopeHeader = function (assigneeCode, initCode, assigneeNode) {
            if (assigneeNode.type === 'Identifier' || assigneeNode.type === 'MemberAccessOp') {
                ...
                newBindings.push.apply(newBindings, __spreadArray([], __read(getAssigneeBindings(assigneeNode))));
                return paramName;
            }
        };
        parameter.patch();
        ...
        return {
            newAssignments: __spreadArray(__spreadArray([], __read(defaultParamAssignments)), __read(thisAssignments)),
