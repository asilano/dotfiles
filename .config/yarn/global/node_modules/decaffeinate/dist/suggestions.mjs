...
export var FIX_INVALID_CONSTRUCTOR = {
...
export function prependSuggestionComment(code, suggestions) {
    ...
    var commentLines = __spreadArray(__spreadArray([
        '/*',
        ' * decaffeinate suggestions:'
    ], __read(suggestions.map(function (_a) {
    ...
    if (codeLines[0].startsWith('#!')) {
        return __spreadArray(__spreadArray([codeLines[0]], __read(commentLines)), __read(codeLines.slice(1))).join('\n');
    }
    else {
        return __spreadArray(__spreadArray([], __read(commentLines)), __read(codeLines)).join('\n');
    }
}
